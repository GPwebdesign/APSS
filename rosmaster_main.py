...
def _load_calibration():
    try:
        with open(_CAL_PATH) as f:
            return _json.load(f)['motors']
    except Exception:
        return {'m1': 1.0, 'm2': 1.0, 'm3': 1.0, 'm4': 1.0}


from gevent import pywsgi

# ─────────────────────────────────────────────
# APSS — Aggiunta odometria ROS2 (Apr 2026)
# Importa rclpy e messaggi necessari per
# pubblicare /odom e il tf odom→base_footprint
# ─────────────────────────────────────────────
try:
    import sys as _sys
    _ROS2_PATH = '/opt/ros/humble/local/lib/python3.10/dist-packages'
    ...
    print("[ODOM] rclpy non disponibile — odometria disabilitata")
...
# 摄像头库
#g_camera = Rosmaster_Camera(video_id=0x50, debug=g_debug) #questo era per x3 plus con camera usb
g_camera = Rosmaster_Camera(video_id=0x53, debug=g_debug)   # questo è per x3 plus con camera CSI 
...
# ─────────────────────────────────────────────
# APSS — Variabili globali odometria (Apr 2026)
# Posizione e orientamento stimati del robot
# aggiornati dal thread odometria
# ─────────────────────────────────────────────
g_odom_x     = 0.0   # posizione X nel frame odom (metri)
g_odom_y     = 0.0   # posizione Y nel frame odom (metri)
g_odom_theta = 0.0   # orientamento (radianti)
g_enc_prev   = None  # valori encoder al ciclo precedente
g_image_pub  = None
g_ros_node   = None
g_latest_frame = None
g_frame_lock = threading.Lock()

# Parametri cinematica Mecanum R2 (misurati fisicamente)
WHEEL_RADIUS       = 0.0395  # raggio ruota (m) — diametro 79mm misurato
ENCODER_CPR        = 822.0   # impulsi/giro — misurato fisicamente su M2
WHEEL_SEPARATION_X = 0.078   # semidistanza asse longitudinale (m) — misurato 78mm
WHEEL_SEPARATION_Y = 0.105   # semidistanza asse laterale (m) — misurato 105mm
...
def parse_data(sk_client, data):
    ...
    if cmd == "0F":  # 进入界面
    ...
    elif cmd in ("1A", "1a"):  # cinematica Mecanum custom — set_motor diretto
        m1 = int(data[7:9], 16)
        m2 = int(data[9:11], 16)
        ...
        m3 = int(m3 * cal['m3'])
        m4 = int(m4 * cal['m4'])
        g_bot.set_motor(m1, m2, m3, m4)
        if g_debug:
            print(f"set_motor: {m1}, {m2}, {m3}, {m4}")
...
# ─────────────────────────────────────────────────────
# APSS — Publisher frame camera su ROS2 (Apr 2026)
# Pubblica i frame della camera su /image_raw
# permettendo ai nodi ROS2 di obstacle avoidance
# di usare la camera senza conflitti con lo stream MJPEG
# ─────────────────────────────────────────────────────
def publish_image_frame(frame, pub, node):
    """Pubblica frame numpy BGR su topic ROS2 /image_raw — ridimensionato a 320x240."""
    try:
        if not ROS2_AVAILABLE or pub is None:
            return
        # Ridimensiona a 320x240 per ridurre carico CPU/rete
        small = cv.resize(frame, (160, 120))
        img_msg = RosImage()
        img_msg.header.stamp    = node.get_clock().now().to_msg()
        img_msg.header.frame_id = 'camera_link'
        img_msg.height  = small.shape[0]
        img_msg.width   = small.shape[1]
        img_msg.encoding = 'bgr8'
        img_msg.step    = small.shape[1] * 3
        img_msg.data    = small.tobytes()
        pub.publish(img_msg)
    except Exception as e:
        if g_debug:
            print(f"[IMAGE_PUB] Errore: {e}")
...
# ─────────────────────────────────────────────
# APSS — Thread odometria ROS2 (Apr 2026)
# Legge encoder Yahboom ogni 50ms, calcola
# posizione con cinematica Mecanum inversa,
# pubblica /odom e tf odom→base_footprint.
#
# Cinematica inversa Mecanum (verificata su APSS):
#   vx = ( enc1 + enc2 + enc3 + enc4) / 4
#   vy = (-enc1 + enc2 + enc3 - enc4) / 4  [non usato per ora]
#   vz = ( enc1 - enc2 + enc3 - enc4) / 4
#
# Nota: M1 e M4 hanno polarità invertita fisicamente —
# gli encoder restituiscono valori negativi in avanti
# quindi vanno negati prima del calcolo
# ─────────────────────────────────────────────
def thread_camera():
    # ─────────────────────────────────────────────
    # APSS — Thread cattura camera (Apr 2026)
    # Cattura frame dalla camera in modo autonomo
    # indipendente dalla connessione app Kivy.
    # Alimenta sia lo stream MJPEG che /image_raw ROS2
    # ─────────────────────────────────────────────
    global g_latest_frame, g_camera_type
    print("[CAMERA] Thread cattura camera avviato")
    while True:
        try:
            if g_camera_type == g_camera.TYPE_USB_CAMERA:
            ...
            if g_debug:
                print(f"[CAMERA] Errore cattura: {e}")
            time.sleep(0.5)


def thread_odom():
    global g_odom_x, g_odom_y, g_odom_theta, g_enc_prev

    if not ROS2_AVAILABLE:
        print("[ODOM] ROS2 non disponibile — thread odometria non avviato")
        return

    # Inizializza nodo ROS2
    rclpy.init()
    node = Node('apss_odom_publisher')
    odom_pub = node.create_publisher(Odometry, '/odom', 10)
    # ── Publisher immagine camera per obstacle avoidance ──
    image_pub = node.create_publisher(RosImage, '/image_raw', 10)
    print("[ODOM] Publisher /image_raw avviato")
    global g_image_pub, g_ros_node
    g_image_pub = image_pub
    g_ros_node  = node
    tf_broadcaster = tf2_ros.TransformBroadcaster(node)

    print("[ODOM] Thread odometria avviato")

    DT = 0.05  # ciclo 50ms = 20Hz

    while True:
        try:
            # ── Lettura encoder ──────────────────────────
            e1, e2, e3, e4 = g_bot.get_motor_encoder()

            # M1 e M4 hanno polarità invertita fisicamente
            # negazione per coerenza con cinematica
            e1 = -e1
            e4 = -e4

            # Inizializzazione al primo ciclo
            if g_enc_prev is None:
                g_enc_prev = (e1, e2, e3, e4)
                time.sleep(DT)
                continue

            # ── Calcolo delta encoder ────────────────────
            de1 = e1 - g_enc_prev[0]
            de2 = e2 - g_enc_prev[1]
            de3 = e3 - g_enc_prev[2]
            de4 = e4 - g_enc_prev[3]
            ...
            # ── Cinematica Mecanum inversa ───────────────
            # Converti impulsi encoder in metri
            # (delta_enc / CPR) * 2*pi*r
            k = (2.0 * math.pi * WHEEL_RADIUS) / ENCODER_CPR
            d1 = de1 * k
            d2 = de2 * k
            d3 = de3 * k
            d4 = de4 * k

            # Velocità lineari e angolare dal robot
            vx = ( d1 + d2 + d3 + d4) / 4.0
            vy = (-d1 + d2 + d3 - d4) / 4.0
            vz = ( d1 - d2 + d3 - d4) / (
            ...
            delta_theta = vz
            delta_x = (vx * math.cos(g_odom_theta)
                      - vy * math.sin(g_odom_theta))
            delta_y = (vx * math.sin(g_odom_theta)
                      + vy * math.cos(g_odom_theta))

            g_odom_x     += delta_x
            g_odom_y     += delta_y
            g_odom_theta += delta_theta

            # ── Quaternione da yaw ───────────────────────
            qz = math.sin(g_odom_theta / 2.0)
            qw = math.cos(g_odom_theta / 2.0)
            ...
            odom_msg.twist.twist.angular.z = vz / DT
            ...
            tf_msg.transform.translation.x = g_odom_x
            tf_msg.transform.translation.y = g_odom_y
            ...
            if g_debug:
                print(f"[ODOM] Errore: {e}")

        time.sleep(DT)


def thread_image_publisher():
    """Thread dedicato: legge g_latest_frame e pubblica su /image_raw a 10Hz."""
    print("[IMAGE_PUB] Thread publisher /image_raw avviato")
    while True:
        try:
            with g_frame_lock:
                frame = g_latest_frame
            if frame is not None:
                publish_image_frame(frame, g_image_pub, g_ros_node)
        except Exception as e:
            if g_debug:
                print(f"[IMAGE_PUB] Errore: {e}")
...
if __name__ == '__main__':
    task_camera = threading.Thread(
        target=thread_camera, name="task_camera")
    task_camera.setDaemon(True)
    task_camera.start()
    print("[CAMERA] Thread camera avviato")

    # ─────────────────────────────────────────────
    # APSS — Centratura pan/tilt all'avvio (Apr 2026)
    # Porta i servo alla posizione home definita in
    # pan_tilt_presets.json prima di accettare connessioni
    # ─────────────────────────────────────────────
    try:
        _pt_path = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                'pan_tilt_presets.json')
        with open(_pt_path) as _f:
            _presets = _json.load(_f)
        _pan_home  = _presets['home']['pan']
        _tilt_home = _presets['home']['tilt']
        time.sleep(0.5)
        g_bot.set_pwm_servo(2, _pan_home)
        time.sleep(0.1)
        g_bot.set_pwm_servo(1, _tilt_home)
        print(f"[PAN_TILT] Home impostato: pan={_pan_home} tilt={_tilt_home}")
    except Exception as _e:
        print(f"[PAN_TILT] Errore home: {_e}")
    ...
    # ─────────────────────────────────────────
    # APSS — Avvio thread odometria (Apr 2026)
    # Pubblica /odom e tf odom→base_footprint
    # tramite lettura encoder Yahboom
    # ─────────────────────────────────────────
    if ROS2_AVAILABLE:
        task_odom = threading.Thread(
            target=thread_odom, name="task_odom")
        task_odom.setDaemon(True)
        task_odom.start()
        print("[ODOM] Thread odometria avviato")
        task_image = threading.Thread(
            target=thread_image_publisher, name="task_image")
        task_image.setDaemon(True)
        task_image.start()
        print("[IMAGE_PUB] Thread publisher /image_raw avviato")
    else:
        print("[ODOM] ROS2 non disponibile — odometria disabilitata")
